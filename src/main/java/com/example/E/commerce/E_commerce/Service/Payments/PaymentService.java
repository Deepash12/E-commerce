...
public class PaymentService
{
    ...
    // ─────────────────────────────────────────────────────────────
    // STEP 1 — User ne "Pay Now" click kiya
    // Sirf Payment record banta hai, order nahi
    // ─────────────────────────────────────────────────────────────
    public PaymentResponseDTO initiatePayment(CheckoutOrderResponseDTO dto, PaymentMethod method)
    {
        String username = Objects.requireNonNull(
        ...
        payment.setTransactionId(createPaymentOrder(dto.getAmount()));
        payment.setExpiresAt(LocalDateTime.now().plusMinutes(15));
        // NOTE: order intentionally null — set hoga sirf payment success hone par
    ...
    // Success → Order banao + confirm karo + cart clear karo
    // Failure → sirf failure count badhaao
    // ─────────────────────────────────────────────────────────────
    public Payment completePayment(Long paymentId)
    {
        try
        {
            ...
            if (success)
            {
                // Payment success — ab order banao, confirm karo, cart clear karo
                Order order = orderService.placeOrderAfterPayment(
                        payment.getAddress().getId(),
                        payment.getPaymentMethod()
                );

                // Payment se order link karo
                payment.setOrder(order);
                payment.setStatus(PaymentStatus.SUCCESS);
            ...
            {
                int newCount = (payment.getFailureCount() == null ? 0 : payment.getFailureCount()) + 1;
